...
function GetLoadNav() {
    ...
    $.each(data, function (i) {
        var row = data[i];
        if (row.F_ParentId == "0") {
            if (row.F_Icon == 'fa fa-power-off') {
                _html += '<li>';
                _html += '<a data-id="out" href="/Login/OutLogin" ><i class="fa fa-power-off"></i><span>' + row.F_FullName + '</span></a>';
                _html += '</li>';
            }
            else {
                _html += '<li>';
...
function fnSearch() {
    var txtSearch = $.trim($('#txtSearch').val().toString());
    if (txtSearch != '') {
        $(".menuItem:contains('" + txtSearch + "')").trigger("click");
    }
}
$(function () {
    top.$("#user-left-box .Portrait").bind("click", function () {
        top.$("#userAvatar").trigger("click");
    });
    top.$(".Portrait").one("error", function (e) {
        $(this).attr("src", "/Content/img/ArchivesHeader.jpg");
    });
});
