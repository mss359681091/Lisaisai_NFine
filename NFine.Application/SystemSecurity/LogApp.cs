    ...
    public class LogApp
    {
        ...
        public List<LogEntity> GetList(Pagination pagination, string queryJson)
        {
            ...
            if (!queryParam["keyword"].IsEmpty())
            {
                ...
                expression = expression.And(t => t.F_Account.Contains(keyword) || t.F_NickName.Contains(keyword) || t.F_ModuleName.Contains(keyword));
            }
            if (!queryParam["fType"].IsEmpty())
            {
                string fType = queryParam["fType"].ToString();
                expression = expression.And(t => t.F_Type.Equals(fType));
            }
