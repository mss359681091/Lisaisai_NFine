...
namespace Client.Common
{
    public class WebApiHelper
    {
        ...
        /// <returns></returns>
        public static T Post<T>(string url, string data, string staffId, string GetTokenApi)
        {
            ...
            request.Headers.Add("signature", GetSignature(timeStamp, nonce, staffId, data, GetTokenApi)); //当前请求内容的数字签名
        ...
        /// <param name="staffId"></param>
        /// <returns></returns>
        public static T Get<T>(string webApi, string query, string queryStr, string staffId, string GetTokenApi, bool sign = true)
        {
            ...
            if (sign)
                request.Headers.Add("signature", GetSignature(timeStamp, nonce, staffId, query, GetTokenApi)); //当前请求内容的数字签名
        ...
        /// <returns></returns>
        public static TokenResultMsg GetSignToken(string staffId, string GetTokenApi)
        {
            ...
            TokenResultMsg token = WebApiHelper.Get<TokenResultMsg>(tokenApi, parameters.Item1, parameters.Item2, staffId, GetTokenApi, false);
            return token;
        }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        private static string GetSignature(string timeStamp, string nonce, string staffId, string data, string GetTokenApi)
        {
            Token token = null;
            var resultMsg = GetSignToken(staffId, GetTokenApi);
            if (resultMsg != null)
            {
