            ...
            get
            {
                if (_scheduler == null)
                {
                    lock (Locker)
                    {
                        if (_scheduler == null)
                        {
                            string strip = ConfigurationManager.AppSettings["iplist"];//读取配置文件IP集合
                            string curip = string.Empty;
                            if (!string.IsNullOrEmpty(strip))
                            {
                                curip = IpHelper.getCurIp(strip, Convert.ToInt32(port));//获取一个可用ip
                            }
                            else
                            {
                                curip = "127.0.0.1";//如果配置文件没有配置ip,则使用默认
                            }
